...
uint8_t strobe_state(Event event, uint16_t arg) {
    ...
    // pass all events to beacon-mode-isg, when it's active 
    if (st == beacon_mode_isg_e) {
        beacon_mode_isg_state(event, arg);
    }
    #endif
    /// beacon-mode-isg
    ...
    #endif    /// #if defined(USE_LIGHTNING_MODE) || defined(USE_CANDLE_MODE)
    /// #if defined(USE_LIGHTNING_MODE) || defined(USE_CANDLE_MODE)
