...
/// LIGHTHOUSE MODE ADJUSTMENTS

    /// 4C // ... decrease lighthouse delay by 1 sec 
    /// 5C // ... increasing lighthouse delay by 1 sec
    /// 6C // ... reset lighthouse delay to default
...
/// MY REMAP BUTTON CLICKS, HOLD, etc.
...
uint8_t strobe_state(Event event, uint16_t arg) {
    ...
    #ifdef USE_BEACON_MISG
    // pass all events to beacon_in_strobe mode, when it's active
    if (st == beacon_misg_e) {
        beacon_misg_state(event, arg);
    }
    #endif
   ...
   /// OG:   // 3 clicks: rotate through channel modes for the current strobe
    ...
    #endif   /// end of #ifdef USE_MOMENTARY_MODE
...
inline void strobe_state_iter() {
    ...
    switch(st) {
        ...
        #ifdef USE_BEACON_IN_STROBE_GROUP
        case beacon_in_strobe_group_e:
            beacon_in_strobe_iter();
            break;
        #endif
         */
...
/*   /// 2025/02/05 17:57
/// beacon-mode in strobes group 

#ifdef USE_BEACON_IN_STROBE_GROUP

inline void beacon_in_strobe_iter() {
    // one iteration of main loop()
...
/// 2025/02/07  beacon-in-strobe-group
#ifdef USE_BEACON_MISG
#include "anduril/beacon-misg.c"
#endif
